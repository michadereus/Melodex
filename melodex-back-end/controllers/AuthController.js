...
async function exportPlaylistStub(req, res) {
  ...
  if (
    exportStubEnabled() &&
    Array.isArray(uris) &&
    !Array.isArray(__testUris) // never legacy-ack when __testUris is used
  ) {
        ...
        for (const it of visible) {
          ...
          if (typeof u === "string" && u.length > 0) {
            ...
            if (typeof next === "string") {
              ...
              const id = it?.deezerID ?? it?.deezerId ?? it?.id ?? null;
              if (id != null) mappedUris.push(`spotify:track:${id}`);
            }
          }
        }
  ...
  try {
    ...
    return res
      .status(200)
      .json({ ok: true, received: { name: name ?? null, count } });
  }
}
