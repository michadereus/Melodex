...
export const SongRanker = ({ mode }) => {
  ...
  useEffect(() => {
    if (wrapperRef.current) {
      wrapperRef.current.style.transform = showFilter
        ? "translateY(15px)"
        : "translateY(0)";
    }
  }, [showFilter]);
    ...
    if (currentPair.length > 0 && !loading && (mode !== "new" || applied)) {
      ...
      fetch(url, {
        ...
        .then((response) => {
          ...
          if (!response.ok) {
            throw new Error(
              `HTTP error enriching songs! Status: ${response.status}`
            );
          }
    ...
    try {
      if (mode === "new") {
        ...
        if (newSongs.length === 0) {
          ...
          if (newSongs.length === 0) {
            setFetchError(
              "Unable to load new songs from the server. Please try again later or contact support."
            );
          }
        }
        ...
        const fetchPromise = fetchReRankingData(
          filters.genre,
          filters.subgenre
        );
      ...
      setFetchError(
        "An error occurred while fetching songs. Please try again or contact support."
      );
      setApplied(true);
    } finally {
      setLoading(false);
    ...
    <div className="song-ranker-container">
      ...
      <div
        className={`filter-container ${showFilter ? "visible" : "hidden"} ${
          mode === "new" ? "rank-mode" : "rerank-mode"
        }`}
        style={{ margin: "0 auto" }}
      >
        <SongFilter
      ...
      {/* FILTER TOGGLE — same behavior as /rankings */}
      <div
        style={{
          display: "flex",
      ...
      {isProcessing || (loading && mode === "rerank") ? (
        ...
        <div className="song-ranker-wrapper" ref={wrapperRef}>
          <h2
            style={{
              textAlign: "center",
              color: "#141820",
          ...
          >
            {mode === "new"
              ? selectedGenre
                ? `Rank ${selectedGenre} Songs`
                : "Rank All Songs"
              : selectedGenre
              ? `Rerank ${selectedGenre} Songs`
              : "Re-rank All Songs"}
          </h2>
