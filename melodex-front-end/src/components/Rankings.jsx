...
const Rankings = () => {
  ...
  // Fetch enriched song data (Deezer info) when rankedSongs updates
  useEffect(() => {
    if (applied && rankedSongs.length > 0) {
      fetch(`${API_BASE_URL}/user-songs/deezer-info`, {
        ...
        .catch((error) => {
          ...
          setEnrichedSongs(rankedSongs); // Fallback to un-enriched songs
        });
    }
  }, [rankedSongs, applied]);
  ...
  const handleApply = async (filters) => {
    setApplied(true);
    await fetchRankedSongs(filters.genre, filters.subgenre); // Sole fetch call
  };
  ...
  return (
    <div style={{ width: '100%' }}>
      <h2 style={{ textAlign: 'center', color: '#2c3e50', fontSize: '2rem', marginBottom: '2rem' }}>Your Song Rankings</h2>
        ...
        <ul style={{ listStyle: 'none', padding: 0, display: 'grid', gap: '1.5rem', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', width: '100%' }}>
          {rankedWithPositions.map((song) => (
            <li key={song.deezerID} style={{ background: 'white', borderRadius: '12px', padding: '1.5rem', boxShadow: '0 4px 12px rgba(0, 0, 0, 0.1)', display: 'flex', alignItems: 'center', gap: '1rem', position: 'relative' }}>
              <span style={{ fontSize: '1.5rem', fontWeight: '700', color: '#3498db', minWidth: '2rem', textAlign: 'center' }}>{song.position}</span>
              <img src={song.albumCover} alt="Album Cover" style={{ width: '80px', height: '80px', borderRadius: '8px' }} />
              <div style={{ flex: 1 }}>
                <p style={{ fontSize: '1.1rem', fontWeight: '600', color: '#2c3e50', margin: '0' }}>{song.songName}</p>
                ...
                <audio controls src={song.previewURL} style={{ width: '100%', marginTop: '0.5rem' }} onError={(e) => { console.debug('Audio preview unavailable:', song.songName); e.target.style.display = 'none'; e.target.nextSibling.style.display = 'block'; }} />
